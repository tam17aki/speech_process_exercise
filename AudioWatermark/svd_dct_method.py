...
"""A python script to perform watermark embedding/detection
   on the basis of singular value decomposition (SVD) to DCT coefficients."""
...
def embed():
    ...
    # DCT係数を抽出
    pointer = 0
    dct_mat = np.zeros((n_frames, FRAME_LENGTH))
    for i in range(n_frames):
        frame = host_signal[pointer: (pointer + FRAME_LENGTH)]

        # dct係数
        dct_mat[i, :] = scipy.fft.dct(frame, n=FRAME_LENGTH)

        pointer = pointer + frame_shift

    # ケプストラム行列を特異値分解
    U, D, V = np.linalg.svd(dct_mat, full_matrices=False)
    ...
    # 再構築
    wmed_dct = U @ np.diag(Dw) @ V

    # 透かし入りDCT係数を音声に戻す
    wmed_signal = np.zeros((frame_shift * n_frames))  # watermarked signal
    for i in range(n_frames):
        # 逆変換
        wmarked_frame = scipy.fft.idct(wmed_dct[i, :])
...
def detect():
    ...
    # DCT係数を抽出
    pointer = 0
    wmed_dct_mat = np.zeros((n_frames, FRAME_LENGTH))
    for i in range(n_frames):
        frame = eval_signal[pointer: (pointer + FRAME_LENGTH)]

        # DCT係数
        wmed_dct_mat[i, :] = scipy.fft.dct(frame, n=FRAME_LENGTH)
    ...
    # DCT係数行列を特異値分解
    U, Dw, V = np.linalg.svd(wmed_dct_mat, full_matrices=False)
