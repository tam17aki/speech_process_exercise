...
def embed():
    ...
    # 埋め込みの総ビット数
    embed_nbit = np.min([stft_mat.shape[0], stft_mat.shape[1]])
...
def detect():
    ...
    W = (1.0 / alpha) * W
    W = W / D
    W = W[:embed_nbit, :embed_nbit]

    detected_bit = np.zeros((embed_nbit, embed_nbit))
    for i in range(embed_nbit):
        for j in range(embed_nbit):
            if W[i, j] > THRESHOLD:
                detected_bit[i, j] = 1
    ...
    for i in range(effective_nbit):
        for j in range(effective_nbit):
            ...
            ave = np.mean(detected_bit[i * NUM_REPS: (i + 1) * NUM_REPS,
